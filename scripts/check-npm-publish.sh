...
[ -r "$REGISTRY_LAUNCHER" ] || fatal \
  ...
  "restore that file with 'git checkout -- scripts/loopback-npm-registry.py' and rerun"
...
python3 "$REGISTRY_LAUNCHER" "$PUBLISHED" "$PORT_FILE" "$MODE_FILE" 2>"$REGISTRY_LOG" &
REGISTRY_PID=$!
...
[ -z "$malformed" ] || {
  ...
  fatal \
    ...
    "repair do_PUT in scripts/loopback-npm-registry.py so each case above is answered 400"
}
