...
///
/// Version 11 records comments on a task: the contract's `Comment` and `NewComment`, the
/// page a plugin answers `task_comments` with, the two shapes the comment verbs answer with,
/// `TaskDetail` for `task show`, and `Capabilities` gaining `comments` — which moves every
/// root that reaches it.
const ELEVENTH_BUNDLE_SHAPE: [(&str, u64); 64] = [
